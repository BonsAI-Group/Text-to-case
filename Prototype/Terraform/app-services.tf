...
resource "azurerm_linux_web_app" "frontend" {
  ...
  app_settings = {
    ...
    REACT_APP_API_URL                   = "https://webapp-${local.resource_prefix}-backend.azurewebsites.net"
  }
  site_config {
    always_on                           = true
...
resource "azurerm_linux_web_app" "backend" {
  ...
  app_settings = {
    ...
    DOCKER_REGISTRY_SERVER_PASSWORD     = ""
    WEBSITES_ENABLE_APP_SERVICE_STORAGE = false
    FRONTEND_URL                        = "https://webapp-${local.resource_prefix}-frontend.azurewebsites.net"
    MODEL_URL                           = "https://webapp-${local.resource_prefix}-models.azurewebsites.net"
  }
  site_config {
    always_on                           = true
    minimum_tls_version                 = "1.2"
